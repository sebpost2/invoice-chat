  ...
  return (
    <details className="group rounded-md border border-zinc-800 bg-zinc-950/60 text-xs overflow-hidden">
      ...
      <div className="border-t border-zinc-800 px-3 py-2 space-y-2 font-mono text-[11px] text-zinc-400">
        {part.input != null && (
          <div>
            <div className="text-zinc-500 mb-0.5">{t.tool.input}</div>
        ...
        {isDone && (
          <div>
            <div className="text-zinc-500 mb-0.5">{t.tool.output}</div>
        ...
        {isError && (
          <div className="text-red-400">
            {part.errorText ?? t.tool.errorFallback}
          </div>
        )}
      </div>
    </details>
      ...
      >
        {messages.length === 0 && (
          <div className="space-y-4 text-sm">
            <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4">
              <p className="text-zinc-300 font-medium mb-1">{t.intro.title}</p>
              <p className="text-xs text-zinc-500">{t.intro.body}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {t.intro.suggestions.map((s) => (
                <button
                  key={s}
                  onClick={() => submit(s)}
                  disabled={busy}
              ...
              >
                {m.parts.map((part, idx) => {
                  ...
                  if (part.type.startsWith("tool-")) {
                    return <ToolDetail key={idx} part={part as ToolPart} />;
                  }
                  return null;
                })}
        ...
        {error && (
          <div className="rounded-md border border-red-900/40 bg-red-950/30 text-red-300 text-sm px-3 py-2">
            {error.message || t.input.genericError}
          </div>
        )}
      </div>
